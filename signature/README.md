...
| `preview.html` | Local test harness, six render states. **Not shipped, never pasted anywhere.** |
...
or delete it. Any future change ships as `-v2` with a new URL.
...
Still deployed and still serving. Nothing references it since the v2 redesign,
but **every email sent before that redesign still points at it**. Deleting it
would retroactively break the logo in mail already sitting in people's inboxes.
Leave it in place.
...
Open `preview.html` in Chrome. Six states, each stage 620px wide:
...
6. **`bgcolor` stripped** — the worst case. See below.
...
| Tag | Commit | Design |
|---|---|---|
| `email-signature-v2` | `20395fc` | Current — 600px navy block, cream reverse logo |
